...
public class HttpClient {
    ...
    /**
     * 重载post请求方法，该方法可以设置请求头的信息.　<br/>
     * 发送HTTP_POST请求 type: 默认是表单请求，
     */
    public static String sendPostRequest(String reqUrl, String param, String type, String headerParam) {
        String result = RESP_CONTENT;
        // 设置请求和传输超时时间
        HttpPost httpPost = new HttpPost(reqUrl);
        // 这就有可能会导致服务端接收不到POST过去的参数,比如运行在Tomcat6.0.36中的Servlet,所以我们手工指定CONTENT_TYPE头消息
        ...
        logger.debug("获取的Authorization：" + ("Basic " + Base64.encodeBase64(headerParam.getBytes())));
        httpPost.setHeader("Authorization", "Basic " + Base64.encodeBase64(headerParam.getBytes()));
        CloseableHttpResponse response = null;
        try {
            if (param != null) {
                StringEntity entity = new StringEntity(param, ENCODE_CHARSET);
